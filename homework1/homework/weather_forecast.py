...
class WeatherForecast:
    ...
    def find_the_largest_drop(self) -> torch.Tensor:
        ...
        day_diff = day_avg[1:] - day_avg[:-1]
        largest_drop = day_diff.min()
        return largest_drop
        #raise NotImplementedError
    ...
    def find_the_most_extreme_day(self) -> torch.Tensor:
        ...
        """
        day_avg = self.data.mean(dim=1, keepdim=True)
        abs_diff = torch.abs(self.data - day_avg)
        idx = abs_diff.argmax(dim=1)
        day_extreme = self.data[torch.arange(self.data.size(0)), idx]
        return day_extreme
        #raise NotImplementedError
    ...
    def predict_temperature(self, k: int) -> torch.Tensor:
        ...
        """
        predicted_temp = self.data[-k:].mean(dim=0)
        return predicted_temp.mean()
        #raise NotImplementedError
    ...
    def what_day_is_this_from(self, t: torch.FloatTensor) -> torch.LongTensor:
        ...
        """
        t = t.view(1, -1)
        distances = torch.sum(torch.abs(self.data - t), dim=1)
        closest_day_idx = torch.argmin(distances)
        return closest_day_idx
        #raise NotImplementedError
