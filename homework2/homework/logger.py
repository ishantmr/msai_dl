...
def test_logging(logger: tb.SummaryWriter):
    ...
    for epoch in range(10):
        ...
        train_acc_tensor = torch.cat(metrics["train_acc"]).float().mean().item()
        logger.add_scalar("train_accuracy", train_acc_tensor, global_step)
        ...
        val_acc_tensor = torch.cat(metrics["val_acc"]).float().mean().item()
        logger.add_scalar("val_accuracy", val_acc_tensor, global_step)
